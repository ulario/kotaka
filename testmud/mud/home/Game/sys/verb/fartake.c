...
void main(object actor, string args)
{
...

	{
		...
		if (sizeof(closest) > 1) {
			send_out("You walk over and pick one of them up.\n");
			obj = closest[random(sizeof(closest))];
		} else {
			send_out("You walk over and pick it up.\n");
			obj = closest[0];
		}

		actor->set_x_position(obj->query_x_position());
		actor->set_y_position(obj->query_y_position());
		actor->set_z_position(obj->query_z_position());
