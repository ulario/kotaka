 ...
 *
 * Copyright (C) 2009, 2010, 2011, 2012, 2013  Raymond Jennings
 *
...
void add_upgrade_required(int progid)
{
	if (!hitlist->query_element(progid)) {
		...
		if (!users) {
			CHANNELD->post_message("system", "initd", "wtf?  progid " + progid + " has no inheriter list");
			return;
		}
...
void upgrade_system_2()
{
	...
	while (!libs->empty()) {
		...
		CHANNELD->post_message("system", "initd", "Destructing " + path);
	...
	while (!objs->empty()) {
		...
		CHANNELD->post_message("system", "initd", "Recompiling " + path);
		compile_object(path);
